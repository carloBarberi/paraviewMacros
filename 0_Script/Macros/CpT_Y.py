...
boxPosition = [-1.2, -2, 0.0]
boxLength = [7.2, 4.0, 2.0]
# First slice that will be saved and list of all other slices
firstSlice = 0
yy_slices = list(range(0, 1301, 50))
...
# Properties modified on slice1.SliceType
slice1.SliceType.Origin = [-2.0, firstSlice/1000, 1.25]
...
resultLUT.RescaleTransferFunction(-0.5, 1)

# Rescale transfer function
resultPWF.RescaleTransferFunction(-0.5, 1)

# Rescale 2D transfer function
resultTF2D.RescaleTransferFunction(-0.5, 1, 0.0, 1.0)
...
# Properties modified on resultLUT
resultLUT.RGBPoints = [-0.5, 1.0, 0.0, 1.0, -0.33333349999999995, 0.0, 0.0, 1.0, 0.04761925, 0.0, 1.0, 1.0, 0.23809524999999998, 0.5, 1.0, 0.5, 0.42857124999999996, 1.0, 1.0, 0.0, 0.8095240000000001, 1.0, 0.0, 0.0, 1.0, 1.0, 1.0, 1.0]
...
resultLUTColorBar.Title = 'CpT mean'
...
resultLUTColorBar.CustomLabels = [round(x / 10, 1) for x in range(int(1 * 10), int(-0.5 * 10), int(-0.3 * 10))]


#######################################################################################
# Set the camera perpendicular to Y
...
# reset view to fit data bounds
renderView1.ResetCamera(-1.5, 6.5, -1.2000000476837158, -1.2000000476837158, -3.970460219412056e-19, 2.5, True)
...
text1.Text = 'Y = '+str(firstSlice)+'mm'
...
# save screenshot
SaveScreenshot(current_directory + '/../0_Figures/CpTy_slice/Y_'+str(firstSlice)+'.jpeg', layout1, ImageResolution=[imageSize[0], imageSize[1]],
...
for yy in yy_slices:
    ...
    # Properties modified on slice1.SliceType
    slice1.SliceType.Origin = [1.5, -yy/1000, 1.25]
    ...
    # save screenshot
    SaveScreenshot(current_directory + '/../0_Figures/CpTy_slice/Y_'+str(yy)+'.jpeg', layout1, ImageResolution=[imageSize[0], imageSize[1]],
    OverrideColorPalette='WhiteBackground')
